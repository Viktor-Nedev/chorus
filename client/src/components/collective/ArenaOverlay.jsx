...
// Game Arena: рундове с prompt + таймер, три вида игри (draw / memory /
// blind), гласуване или AI съдия, точки, scoreboard между рундовете и
// финален подиум.
const KIND_INFO = {
  draw: { icon: '🎨', title: 'Draw the prompt!' },
  memory: { icon: '🧠', title: 'Memory round — memorize, then draw!' },
  blind: { icon: '🙈', title: 'Blind round — your ink is invisible!' },
};
...
export function ArenaOverlay({ socket, isCreator, myId }) {
  ...
  // Memory рунд: покажи емоджито голямо първите 4 секунди
  useEffect(() => {
  ...
  if (!arena) {
    // Изчакване: host-ът стартира игрите
    return (
      <div className="absolute inset-0 z-30 flex items-center justify-center pointer-events-none">
        <div className="text-center pointer-events-auto animate-fade-in">
          ...
          <p className="mt-2 text-sm text-gray-400 max-w-sm">
            Timed drawing games. Win rounds, collect points.
            <br />
  ...
  return (
    <>
      {/* ── Рунд банер ── */}
      {arena.phase === 'drawing' && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 rounded-2xl bg-ink-soft/90 border border-accent-violet/60 backdrop-blur px-6 py-3 text-center animate-slide-up">
          <div className="text-[10px] uppercase tracking-[0.3em] text-gray-500">
            {info.icon} Round {arena.round}/{arena.totalRounds} — {info.title}
          </div>
          <div className="font-display font-bold text-white text-xl my-0.5">
            {arena.kind === 'memory' && !showEmoji ? 'Draw it from memory!' : `Draw ${arena.prompt.text}`}
          </div>
          <Countdown endsAt={arena.endsAt} />
        </div>
      )}

      {/* Memory флаш — емоджито, което трябва да запомниш */}
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl overflow-hidden border-2 text-left transition disabled:cursor-not-allowed ${
                      chosen
                        ? 'border-accent-violet scale-[1.02]'
                        : mine
                          ? 'border-ink-line opacity-60'
                          : 'border-ink-line hover:border-accent-violet/70'
                    }`}
                  >
              ...
              {(arena.results || []).map((r, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl border p-2.5 ${
                    i === 0 ? 'border-yellow-600 bg-yellow-950/20' : 'border-ink-line'
                  }`}
                >
                  ...
                  {r.png && (
                    <img src={r.png} alt="" className="w-20 aspect-video object-cover rounded bg-ink" />
                  )}
                  <span className="flex-1 text-sm text-white truncate">
                    {r.userId === myId ? 'you' : r.nickname}
                  </span>
