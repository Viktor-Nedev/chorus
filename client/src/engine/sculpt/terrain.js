...
export const TERRAIN_SEGS = 150; // сегменти на страна
...
function heightColor(t, out) {
  for (let i = 1; i < STOPS.length; i++) {
    if (t <= STOPS[i].h) {
      ...
      return out.copy(a.c).lerp(b.c, (t - a.h) / (b.h - a.h));
    }
  }
...
export function createTerrainMesh(params) {
  ...
  const heights = p.heights ? Float32Array.from(p.heights) : generateHeights(p);
...
export function sculptTerrain(mesh, point, { mode = 'raise', radius = 3, strength = 1, dt = 0.016 }) {
  ...
  const cx = ((local.x + TERRAIN_SIZE / 2) / TERRAIN_SIZE) * TERRAIN_SEGS;
  const cy = ((local.y + TERRAIN_SIZE / 2) / TERRAIN_SIZE) * TERRAIN_SEGS;
...
export function terrainHeightAt(mesh, wx, wz) {
  ...
  const n = TERRAIN_SEGS + 1;
  const gx = ((wx + TERRAIN_SIZE / 2) / TERRAIN_SIZE) * TERRAIN_SEGS;
  const gy = ((-wz + TERRAIN_SIZE / 2) / TERRAIN_SIZE) * TERRAIN_SEGS; // z → -y при ротацията
...
function makeScatterTemplate(kind) {
  ...
  // grass — две кръстосани плоскости
  const p1 = new THREE.PlaneGeometry(0.32, 0.34);
  p1.translate(0, 0.17, 0);
  const p2 = p1.clone();
  p2.rotateY(Math.PI / 2);
  const merged = mergeGeometries([
    { geo: p1, color: new THREE.Color('#4c9b52') },
    { geo: p2, color: new THREE.Color('#5cab5e') },
  ]);
  return {
    geometry: merged,
    material: new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.9, side: THREE.DoubleSide }),
  };
}
