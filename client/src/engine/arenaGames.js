// Портнат от server/services/arenaGames.js — Game Arena логиката вече върви
// в браузъра на host-а (Supabase Realtime няма сървърна част).
// Чисти функции → същите unit тестове както на сървъра.
...
export function buildRoundPlan(totalRounds, playerCount) {
  const eligible = (k) =>
...
export const GAME_CHOICES = ['mixed', 'draw', 'pictionary', 'memory', 'impostor', 'blind'];
...
export function planFromGame(game, totalRounds, playerCount) {
...
export function isCorrectGuess(text, secret) {
...
export function pictionaryScore(order) {
  return Math.max(40, 100 - order * 20);
}

// Точки за рисуващия: пропорционално на дела познали (0..100).
export function pictionaryDrawerScore(guessedCount, guessers) {
...
export function impostorScores(votes, impostorId, players) {
  const tally = {};
